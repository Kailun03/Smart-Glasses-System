...
function ToolManagement() {
  ...
  const InfoRow = ({ label, value, highlight = false }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontSize: '13px' }}>
      ...
      <span style={{ color: isOffline ? '#ef4444' : highlight ? '#00E5FF'  : '#f8fafc', fontWeight: '600' }}>{value}</span>
    </div>
  );
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'FAILED': return <span className="badge-offline"><AlertTriangle size={12}/> Failed</span>;
      default: return null;
    }
  };
  ...
  return (
    <div className="dashboard-container">
      ...
      <style>{`
        :root { --bg-card: rgba(15, 23, 42, 0.3); --text-primary: #f8fafc; --text-secondary: #94a3b8; --border-color: rgba(255,255,255,0.08); --accent-blue: #00E5FF; --accent-teal: #2dd4bf; --status-active: #10b981; --status-standby: #f59e0b; }
        
        .dashboard-container { height: 100vh; width: 100%; background: transparent; font-family: system-ui, sans-serif; color: var(--text-primary); display: flex; flex-direction: column; padding: 40px; box-sizing: border-box; overflow: hidden; }
        
        /* Layout Grid */
        .tool-layout-grid { display: grid; grid-template-columns: 460px 1fr; gap: 32px; flex: 1; min-height: 0; }
        @media (max-width: 1100px) { .tool-layout-grid { grid-template-columns: 1fr; overflow-y: auto; } }
        
        /* Glassmorphism Cards */
        .widget-card { background: var(--bg-card); backdrop-filter: blur(25px); -webkit-backdrop-filter: blur(25px); border-radius: 24px; border: 1px solid var(--border-color); padding: 24px; display: flex; flex-direction: column; box-shadow: 0 15px 35px rgba(0,0,0,0.4); transition: all 0.3s ease; position: relative; }
        .widget-card:hover { border-color: rgba(0, 229, 255, 0.3); transform: translateY(-2px); box-shadow: 0 20px 40px rgba(0, 229, 255, 0.15); }
        .widget-header { display: flex; align-items: center; gap: 10px; color: var(--text-secondary); font-size: 11px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 24px; }
        .widget-icon { color: var(--accent-blue); }
        .widget-content { flex: 1; display: flex; flex-direction: column; }

        /* Table Styling */
        .table-row { transition: all 0.2s ease; }
        .table-row:hover { background-color: rgba(0, 229, 255, 0.05); box-shadow: inset 3px 0 0 0 var(--accent-blue);}
        
        /* Fix for Issue 1: Inputs Exceeding Bounds */
        .form-input { width: 100%; background: rgba(0,0,0,0.25); border: 1px solid rgba(255,255,255,0.05); color: #f8fafc; padding: 14px 18px; border-radius: 15px; margin-bottom: 20px; box-sizing: border-box; /* This keeps it inside */ font-family: inherit; font-size: 14px; line-height: 1.5; transition: all 0.2s ease; outline: none; box-shadow: inset 0 2px 4px rgba(0,0,0,0.3);}
        .form-input:focus { border-color: var(--accent-blue); background: rgba(0,0,0,0.4); }
        
        .submit-btn { width: 100%; background: var(--accent-blue); color:rgb(37, 43, 59); border: none; padding: 16px; border-radius: 15px; font-weight: 800; font-size: 14px; cursor: pointer; transition: 0.2s; text-transform: uppercase; letter-spacing: 1px; box-shadow: 0 8px 20px rgba(0, 229, 255, 0.3);}
        .submit-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 12px 25px rgba(0, 229, 255, 0.5); }
        .submit-btn:disabled { opacity: 0.7; cursor: not-allowed; }

        /* Badges */
        .badge-active { background: rgba(16, 185, 129, 0.1); color: var(--status-active); border: 1px solid rgba(16, 185, 129, 0.2); padding: 4px 10px; border-radius: 8px; font-size: 11px; font-weight: bold; display: inline-flex; align-items: center; gap: 6px; }
        .badge-processing { background: rgba(0, 229, 255, 0.1); color: var(--accent-blue); border: 1px solid rgba(0, 229, 255, 0.2); padding: 4px 10px; border-radius: 8px; font-size: 11px; font-weight: bold; display: inline-flex; align-items: center; gap: 6px; }
        .badge-standby { background: rgba(245, 158, 11, 0.1); color: var(--status-standby); border: 1px solid rgba(245, 158, 11, 0.2); padding: 4px 10px; border-radius: 8px; font-size: 11px; font-weight: bold; display: inline-flex; align-items: center; gap: 6px; }
        .badge-offline { background: rgba(239, 68, 68, 0.1); color: #ef4444; border: 1px solid rgba(239, 68, 68, 0.2); padding: 4px 10px; border-radius: 8px; font-size: 11px; font-weight: bold; display: inline-flex; align-items: center; gap: 6px; }
        ...
        .file-drop-zone { border: 2px dashed rgba(255,255,255,0.1); border-radius: 20px; padding: 32px 20px; text-align: center; cursor: pointer; background-color: rgba(0,0,0,0.2); display: flex; flex-direction: column; align-items: center; gap: 12px; margin-bottom: 24px; transition: all 0.2s ease;}
        .file-drop-zone:hover { border-color: var(--accent-blue); background-color: rgba(0, 229, 255, 0.05); }

        /* --- NOTIFICATION DROPDOWN --- */
        .notif-bell-wrapper { position: relative; display: flex; align-items: center; justify-content: center; width: 46px; height: 46px; border-radius: 15px; background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255,255,255,0.05); cursor: pointer; transition: all 0.2s; }
        .notif-bell-wrapper:hover { background: rgba(30, 41, 59, 0.8); border-color: rgba(255,255,255,0.1); }
        .notif-badge { position: absolute; top: -5px; right: -5px; background: #ef4444; color: white; font-size: 10px; font-weight: bold; width: 18px; height: 18px; border-radius: 50%; display: flex; justify-content: center; align-items: center; border: 2px solid #070b14; }
        .notif-dropdown { position: absolute; top: 60px; right: 0; width: 360px; background: rgba(15,23,42,0.95); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; box-shadow: 0 20px 40px rgba(0,0,0,0.5); z-index: 50; display: flex; flex-direction: column; overflow: hidden; }
        .notif-header { padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.05); font-weight: 700; display: flex; justify-content: space-between; z-index: 2; }
        ...
        .notif-unread { background: rgba(0, 229, 255, 0.05); }
        .notif-icon { width: 32px; height: 32px; border-radius: 50%; display: flex; justify-content: center; align-items: center; flex-shrink: 0; }
        .n-info { background: rgba(0, 229, 255, 0.1); color: var(--accent-blue); }
        .n-success { background: rgba(16, 185, 129, 0.1); color: var(--status-active); }
        .n-error { background: rgba(239, 68, 68, 0.1); color: var(--status-error); }
        ...
        .drawer-panel { position: fixed; top: 0; right: 0; bottom: 0; width: 450px; background: rgba(15,23,42,0.95); backdrop-filter: blur(25px); border-left: 1px solid rgba(255,255,255,0.08); display: flex; flex-direction: column; z-index: 2001; box-shadow: -20px 0 50px rgba(0,0,0,0.5); animation: slideInRight 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        .drawer-panel.closing { animation: slideOutRight 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
        
        .history-item { padding: 20px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; display: flex; gap: 16px; align-items: flex-start; transition: all 0.2s ease; }
        .history-item:hover { border-color: rgba(0, 229, 255, 0.3); transform: translateX(4px); background: rgba(255,255,255,0.02); }
        ...
        .modal-content-landscape { background: rgba(15,23,42,0.9); backdrop-filter: blur(25px); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; width: 95vw; max-width: 1400px; height: 90vh; display: flex; flex-direction: column; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.6); overflow: hidden; }
        .modal-header-landscape { padding: 20px 32px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.3); flex-shrink: 0; }
        .modal-body-landscape { display: flex; flex: 1; min-height: 0; flex-direction: row; }
        .workspace-wrapper { flex: 3; display: flex; flex-direction: column; background: rgba(0,0,0,0.5); border-right: 1px solid rgba(255,255,255,0.05); }
        ...
        .timeline-container { padding: 20px 32px; background: rgba(0,0,0,0.3); border-top: 1px solid rgba(255,255,255,0.05); flex-shrink: 0; display: flex; flex-direction: column; gap: 16px; }
        .timeline-slider-wrapper { display: flex; align-items: center; gap: 20px; }
        .timeline-slider { -webkit-appearance: none; width: 100%; height: 8px; border-radius: 4px; background: rgba(255,255,255,0.1); outline: none; flex: 1; }
        .timeline-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 20px; height: 20px; border-radius: 6px; background: var(--accent-blue); cursor: pointer; border: 2px solid #0f172a; box-shadow: 0 0 10px rgba(0, 229, 255, 0.4); transition: transform 0.1s; }
        .timeline-slider::-webkit-slider-thumb:hover { transform: scale(1.2); }
        .btn-nav-timeline { padding: 10px 16px; background: rgba(255,255,255,0.05); color: #f8fafc; border: 1px solid rgba(255,255,255,0.1); border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 13px; transition: all 0.2s; min-width: 100px; }
        .btn-nav-timeline:hover:not(:disabled) { background: rgba(255,255,255,0.1); border-color: rgba(0,229,255,0.3); }
        .btn-nav-timeline:disabled { opacity: 0.4; cursor: not-allowed; }
        .interactive-box { position: absolute; border: 2px solid var(--accent-blue); background: rgba(0, 229, 255, 0.15); box-shadow: 0 0 0 9999px rgba(0,0,0,0.6); cursor: move; }
        .resize-handle { position: absolute; width: 12px; height: 12px; background: #fff; border: 2px solid var(--accent-blue); border-radius: 50%; z-index: 10; }
        .resize-handle:hover { transform: scale(1.2); background: var(--accent-blue); }
        ...
        .controls-area { width: 400px; padding: 32px; background: rgba(0,0,0,0.2); display: flex; flex-direction: column; gap: 32px; flex-shrink: 0; overflow-y: auto; }
        .bg-toggle-card { padding: 16px 20px; border-radius: 12px; display: flex; flex-direction: column; gap: 16px; transition: all 0.3s; }
        .readonly-coords { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; background: rgba(0,0,0,0.3); padding: 16px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.05); }
        ...
        .load-more-btn:hover {color: var(--accent-blue); transform: translateX(-4px); }
      `}</style>

      {/* --- RIGHT SIDE DRAWER FOR FULL NOTIFICATION HISTORY --- */}
      {(showAllNotifsModal || isClosingDrawer) && (
        <>
          ...
          <div className={`drawer-panel ${isClosingDrawer ? 'closing' : ''}`}>
            <div style={{ padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(0,0,0,0.3)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div>
                <h3 style={{ fontSize: '14px', color: '#f8fafc', display: 'flex', alignItems: 'center', gap: '12px', margin: 0 }}>
                  <Bell size={18} color="#00E5FF" /> System Notification History
                </h3>
              </div>
              
              <button 
                onClick={handleCloseDrawer} 
                style={{ background: 'none', border: 'none', color: '#f8fafc', cursor: 'pointer', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s', flexShrink: 0 }}
                onMouseOver={(e) => { e.currentTarget.style.background = 'rgba(0, 229, 255, 0.1)'; e.currentTarget.style.color = '#00E5FF'; }} 
            ...
            <div className="custom-scroll" style={{ flex: 1, padding: '24px', background: 'transparent', overflowY: 'auto' }}>
              {notifications.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px', color: '#64748b' }}>No historical notifications found.</div>
              ) : (
        ...
        <div className="modal-overlay">
          <div className="modal-content-landscape animate-slide-up">
            <div className="modal-header-landscape">
              <div>
                <h2 style={{ margin: 0, fontSize: '20px', color: '#f8fafc', display: 'flex', alignItems: 'center', gap: '8px' }}><Maximize size={20} color="#00E5FF" /> Dataset Verification Studio</h2>
              ...
              <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '6px 16px', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold', color: '#00E5FF' }}>Image {currentIndex + 1} of {reviewImages.length}</div>
            </div>
            <div className="modal-body-landscape">
              <div className="workspace-wrapper">
                ...
                <div className="timeline-container">
                  ...
                  <div className="timeline-slider-wrapper">
                    ...
                    <input type="range" min="0" max={reviewImages.length - 1} value={currentIndex} onChange={handleSliderChange} className="timeline-slider" style={{ background: `linear-gradient(to right, #00E5FF ${(currentIndex / (reviewImages.length - 1)) * 100}%, rgba(255,255,255,0.1) ${(currentIndex / (reviewImages.length - 1)) * 100}%)` }} />
              ...
              <div className="controls-area custom-scroll">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div className="bg-toggle-card" style={{ background: reviewImages[currentIndex].isBackground ? 'rgba(16, 185, 129, 0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${reviewImages[currentIndex].isBackground ? 'rgba(16, 185, 129, 0.3)' : 'rgba(255,255,255,0.08)'}` }}>
                    <div>
                      <div style={{ color: '#f8fafc', fontWeight: '600', fontSize: '15px' }}>Negative Mining Match</div>
                      <div style={{ color: '#94a3b8', fontSize: '13px', marginTop: '6px', lineHeight: '1.4' }}>Flag this image as having NO tool present. This is crucial for training the AI to ignore background noise.</div>
                    </div>
                    <button onClick={toggleBackground} style={{ width: '100%', background: reviewImages[currentIndex].isBackground ? '#10b981' : 'rgba(255,255,255,0.1)', color: '#fff', border: '1px solid rgba(255,255,255,0.05)', padding: '12px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px', transition: 'all 0.2s' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <button className="submit-btn" onClick={handleFinalSubmit} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '16px', fontSize: '14px' }}>Approve & Train Model</button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div className="notif-container" ref={notifRef} style={{ position: 'relative'}}>
            <div className="notif-bell-wrapper" onClick={() => setShowNotifs(!showNotifs)}>
              <Bell size={20} color={unreadCount > 0 ? "#f8fafc" : "#94a3b8"} />
              ...
              <div className="notif-dropdown animate-slide-up">
                ...
                <div className="custom-scroll" style={{ maxHeight: '320px', overflowY: 'auto' }}>
                  {displayedNotifs.length === 0 ? (
                    <div style={{ padding: '40px 20px', textAlign: 'center', color: '#64748b', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                      ...
                      <span style={{ fontSize: '13px', fontWeight: '500' }}>No unread notifications.</span>
                    </div>
                  ) : (
                ...
                <div style={{ padding: '12px 20px', borderTop: '1px solid rgba(255,255,255,0.05)', textAlign: 'center' }}>
                  <button 
                    ...
                    style={{ background: 'none', border: 'none', color: '#00E5FF', fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', gap: '6px' }}
                  >
      ...
      <div className="tool-layout-grid">
        <div className="custom-scroll" style={{ display: 'flex', flexDirection: 'column', gap: '32px', overflowY: 'auto', paddingRight: '4px' }}>
          ...
          <WidgetCard title="Register New Tool" icon={UploadCloud} delay="0.3s">
            ...
            <label htmlFor="dataset-upload" className="file-drop-zone">
              <UploadCloud size={36} color={files && files.length > 0 ? "#00E5FF" : "#64748b"} style={{ transition: 'color 0.3s' }} />
              <div style={{ color: '#f8fafc', fontSize: '14px', fontWeight: '600' }}>
                {files && files.length > 0 ? <span style={{ color: '#00E5FF' }}>{files.length} images selected</span> : <span>Click to browse for images</span>}
              </div>
            ...
            <button className="submit-btn" onClick={handleInitiateReview} disabled={isSubmitting}>
              {isSubmitting ? "Queueing Pipeline..." : "Review Annotations & Train"}
            </button>
          </WidgetCard>
        </div>

        {/* Fix for Issue 2: Proper Table Header & Structure Restoration */}
        <div className="widget-card animate-slide-up" style={{ padding: 0, overflow: 'hidden', animationDelay: '0.4s' }}>
          {/* Glass Reflection Highlight for this specific widget */}
          ...
          <div style={{ padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.05)', backgroundColor: 'rgba(0,0,0,0.2)' }}>
            <h2 style={{ margin: 0, fontSize: '16px', color: '#f8fafc', fontWeight: '800', letterSpacing: '0.5px' }}>Trained Tool Repository</h2>
          </div>
          
          <div className="custom-scroll" style={{ flex: 1, overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead style={{ position: 'sticky', top: 0, backgroundColor: 'rgba(15,23,42,0.9)', backdropFilter: 'blur(10px)', zIndex: 1, boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                <tr>
                  <th style={{ padding: '20px 32px', color: '#64748b', fontSize: '11px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.05)', textTransform: 'uppercase', letterSpacing: '1px' }}>Tool Name</th>
                  <th style={{ padding: '20px 32px', color: '#64748b', fontSize: '11px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.05)', textTransform: 'uppercase', letterSpacing: '1px' }}>Class Target</th>
                  <th style={{ padding: '20px 32px', color: '#64748b', fontSize: '11px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.05)', textTransform: 'uppercase', letterSpacing: '1px' }}>Status</th>
                  <th style={{ padding: '20px 32px', color: '#64748b', fontSize: '11px', fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.05)', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'right' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {isLoading && tools.length === 0 ? (
                  ...
                  tools.map((tool) => (
                    <tr key={tool.id} className="table-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                      ...
                      <td style={{ padding: '20px 32px', textAlign: 'right' }}><button onClick={() => handleDeleteTool(tool.id)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', transition: 'color 0.2s' }} onMouseOver={(e)=>e.currentTarget.style.color='#ef4444'} onMouseOut={(e)=>e.currentTarget.style.color='#64748b'} title="Delete"><Trash2 size={16} /></button></td>
                    </tr>
                  ))
                )}
              </tbody>
